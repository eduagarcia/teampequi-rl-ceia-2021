...
if VARIATION == EnvType.multiagent_team:
    observation_space = observation_space*2
...
class MuZeroConfig:
    def __init__(self):
        ...
        ### Game
        self.observation_shape = (1, 1, observation_space)  # Dimensions of the game observation, must be 3D (channel, height, width). For a 1D array, please reshape it to (1, 1, length of array)
        self.action_space = list(range(action_space))  # Fixed list of all possible actions. You should only edit the length
        self.players = list(range(n_players))  # List of players. You should only edit the length
        self.stacked_observations = 0  # Number of previous observations and previous actions to add to the current observation

        # Evaluate
        self.muzero_player = n_players-1  # Turn Muzero begins to play (0: MuZero plays first, 1: MuZero plays second)
        ...
        self.fc_dynamics_layers = [256]  # Define the hidden layers in the dynamics network
        self.fc_reward_layers = [256]  # Define the hidden layers in the reward network
        self.fc_value_layers = [256]  # Define the hidden layers in the value network
        self.fc_policy_layers = [256]  # Define the hidden layers in the policy network
        ...
        self.training_steps = int(1500e3)  # Total number of training steps (ie weights update according to a batch)
...
class Game(AbstractGame):
    ...
    def __init__(self, seed=None):
        ...
        self.reward_buffer = {player_id: 0 for player_id in range(n_players)}
        self.action_buffer = {}
        self.turn = -1
    ...
    def step(self, action):
        ...
        """
        self.turn += 1
        player_id = self.turn % n_players
        self.action_buffer[player_id] = action

        if player_id < n_players-1:  
            return numpy.array([[self.obs_buffer[player_id]]]), self.reward_buffer[player_id], False
        
        observation, reward, done, _ = self.env.step(self.action_buffer)
        self.obs_buffer = observation
        self.reward_buffer = reward

        #print("executed actions:", self.action_buffer, "reward:", reward, "done", done['__all__'])
        return numpy.array([[observation[player_id]]]), reward[player_id], done['__all__']
    ...
    def reset(self):
        ...
        return numpy.array([[observation[0]]])

    def to_play(self):
        return self.turn % n_players
