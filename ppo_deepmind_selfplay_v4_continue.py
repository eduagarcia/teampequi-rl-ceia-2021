...
starting_path = "/home/thiago/Workspace/deepequi-competicao-rl/ray_results/PPO_deepmind_selfplay_v4/PPO_Soccer_ad2cb_00000_0_2021-12-02_23-21-09"
checkpoint_path = starting_path+"/checkpoint_001880/checkpoint-1880"

def file_to_iter(filename):
    return int(filename[filename.find("selfplay_checkpoints/")+len("selfplay_checkpoints/"):filename.rfind(".pkl")])
...
class PrioritizedSelfPlay(DefaultCallbacks):
    ...
    def __init__(self, freq_save = 5, freq_run = 1):
        ...
        files = glob.glob(starting_path + "/selfplay_checkpoints/*")
        self.policy_history = sorted(files, key=file_to_iter)

        print(f"Starting with {len(self.policy_history)} policies")
    ...
    def on_train_result(self, *, trainer, result: dict, **kwargs) -> None:
        ...
        if self.counter % self.freq_save == 0:
            ...
            print(f"Policy Saved: " + weights_filepath)
            self.policy_history.append(weights_filepath)

        #sample opponent policy
        #if self.counter % self.freq_run == 0:
        #50% of the time current policy, 50% random policy
        if np.random.uniform(0, 1) < 0.3 or len(self.policy_history) == 0:
            self.opponent_iter = trainer.iteration
            ...
            opponent_filename = self.policy_history[random_val]
            self.opponent_iter = file_to_iter(opponent_filename)
            with open(opponent_filename, 'rb') as f:
        ...
        print(f"Playing against iteration {self.opponent_iter}\n")
...
if __name__ == "__main__":
    ...
    analysis = tune.run(
        "PPO",
        name="PPO_deepmind_selfplay_v4_continue",
        config={
            "num_gpus": 1,
            "num_workers": 10,
